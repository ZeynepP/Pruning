...
public class MetricSampleQuantiles {
     ...
     * @param v
     */
    synchronized public void insert(double v) {
     ...
     */
    private double query(double quantile) throws IOException {
        if (samples.size() == 0) {
     ...
     */
    synchronized public Map<MetricQuantile, Double> snapshot() throws IOException {
        // flush the buffer first for best results
        insertBatch();
        Map<MetricQuantile, Double> values = new HashMap<MetricQuantile, Double>(quantiles.length);
    ...
    private static class SampleItem {
        ...
        public SampleItem(double value, int lowerDelta, int delta) {
            this.value = value;
            this.g = lowerDelta;
            this.delta = delta;
        }
